...
//!
//! The JS sources reached this through `circomlibjs`'s `eddsa.signPoseidon`,
//! which derives the scalar from a BLAKE-512 hash of a 32-byte private key and
//! then discards the low three bits. Since every key in these benchmarks is
//! freshly random, the derived scalar is sampled directly instead: the
//! resulting signatures satisfy exactly the same verification relation, so the
//! circuits accept them unchanged, and the BLAKE-512 dependency disappears.
