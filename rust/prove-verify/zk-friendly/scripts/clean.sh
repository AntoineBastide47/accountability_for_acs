...
if [[ "$ALL" -eq 1 ]]; then
  # Nuclear: drop artifact folders entirely (summaries too) + the build dir.
  rm -rf "${ARTIFACT_DIRS[@]}" "$ROOT_DIR/target"
else
...
# Generated circuits for the merkle-vs-flat sweep (keeps the benchmark source).
rm -f "$ROOT_DIR/merkle-vs-flat"/flat_t*_u*.circom \
      "$ROOT_DIR/merkle-vs-flat"/merkle_t*_u*.circom 2>/dev/null || true

if [[ "$ALL" -eq 1 ]]; then
  echo "Cleaned artifact folders, generated outputs, and target/."
else
